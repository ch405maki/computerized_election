<template>
    <div class="fixed bottom-0 left-0 w-full z-50 bg-purple-900 border-t shadow-sm px-8 py-2">
      <footer class="flex items-center justify-between space-x-4">
        <span class="text-sm text-white tracking-widest uppercase"> AUSL Election System</span>

        <div class="flex items-center space-x-4 tracking-widest">
            <span class="text-sm text-white">Powered by:</span>
            <img src="/images/logo/itc-logo.png" alt="itc-Logo" class="h-8 w-auto">
            <span class="text-sm text-white border-black">Information Technology Center<br/>
              <p class="text-xs">
                All Rights Reserved &copy; {{ new Date().getFullYear() }}
              </p>
            </span> 
        </div>
      </footer>
  </div>
</template>