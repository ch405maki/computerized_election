...
<template>
    <Sheet defaultOpen @update:open="(val) => !val && $emit('close')">
        <SheetContent side="right" class="w-full overflow-y-auto sm:max-w-md">
            <Form :validation-schema="formSchema" :initial-values="formData" @submit="updateVoter" v-slot="{ meta }" class="mt-4 space-y-4">
                <SheetHeader>
                ...
                <div class="flex justify-end gap-2 pt-4">
                    <Button type="button" variant="outline" @click="$emit('close')" :disabled="isLoading"> Cancel </Button>
