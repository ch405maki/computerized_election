...
watch(selectedElection, (newElectionId) => {
    if (newElectionId) {
        ...
        if (election) {
            positionsInElection.value = [...new Set(election.candidates.filter((c) => c.position).map((c) => c.position!.id))];
        }
    } else {
...
const allPositionsSelected = computed(() => {
    return positionsInElection.value.every((posId) => selectedCandidates.value[posId] !== undefined);
});
...
<template>
    ...
    <AppLayout>
        <div class="space-y-6 p-6">
            ...
            <div v-if="elections.length === 0" class="text-muted-foreground">No active elections available.</div>

            <ElectionSelector v-model="selectedElection" :elections="elections" />
            ...
            <VoteSubmitButton v-if="selectedElection" :is-voting="isVoting" :all-positions-selected="allPositionsSelected" @vote="vote" />
        </div>
    </AppLayout>
</template>
