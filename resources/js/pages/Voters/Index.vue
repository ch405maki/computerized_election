...
import { AlertCircle, CheckCircle, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Info, Loader2, Search, Upload, X } from 'lucide-vue-next';
    ...
    try {
        ...
        if (response.data.queued) {
            setStatus(
                ...
                `Your file with approximately ${estimatedRowCount.value.toLocaleString()} rows is being processed. The page will refresh automatically when done.`,
            );
                ...
                try {
                    ...
                    if (currentStatus === 'completed') {
                        ...
                        setStatus(
                            'error',
                            'Import Failed',
                            'There was an issue processing your file. Please check your column headers and try again.',
                        );
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-4 p-4">
            <div class="flex flex-col gap-4">
                ...
                <div class="flex items-center justify-between gap-2">
                    ...
                    <div class="flex items-center space-x-2">
                        <div class="relative w-full max-w-xs">
                            ...
                            <Input v-model="searchQuery" @input="handleSearch" type="text" placeholder="Search voters..." class="h-9 w-full pl-9" />
                        </div>

                        <input
                            v-if="currentUser?.permissions?.uploadExcel"
                            type="file"
                            ref="fileInput"
                            accept=".xlsx, .xls, .csv"
                            class="hidden"
                ...
                <div v-if="statusTheme" :class="['animate-fade-in flex items-start gap-3 rounded-lg border p-3', statusTheme.bg]">
                    <component :is="statusTheme.icon" :class="['mt-0.5 h-5 w-5', statusTheme.iconColor]" />

                    <div class="flex-1">
                        <h4 :class="['mb-1 font-medium', statusTheme.text]">{{ uploadStatus?.title }}</h4>
                        ...
                        <div v-if="uploadStatus?.type === 'error' && uploadErrors.length" class="mt-2">
                            ...
                            <div v-if="showErrorDetails" class="mt-2 max-h-60 overflow-y-auto rounded border bg-white p-3">
                ...
                <div v-if="processingLargeFile" class="animate-pulse rounded-lg border border-blue-200 bg-blue-50 p-4">
                    <div class="flex items-center gap-3">
                        <Loader2 class="h-5 w-5 animate-spin text-blue-600" />
                        <div>
                            <h4 class="font-medium text-blue-800">Processing large file...</h4>
                            <p class="text-sm text-blue-700">
                                Please wait while we process {{ estimatedRowCount.toLocaleString() }} rows. The page will refresh automatically.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="relative rounded-xl border">
                <div
                    v-if="isFetching"
                    class="absolute inset-0 z-10 flex items-center justify-center rounded-xl bg-white/60 backdrop-blur-[1px] transition-all duration-300"
                >
            ...
            <!-- Pagination -->
            <div v-if="voters.data.length > 0" class="flex flex-col items-center justify-between gap-4 pt-2 sm:flex-row">
                <div class="text-sm text-muted-foreground">
                    Showing <span class="font-medium text-foreground">{{ voters.from }}</span> to
                    <span class="font-medium text-foreground">{{ voters.to }}</span> of
                    <span class="font-medium text-foreground">{{ voters.total }}</span> results
                </div>
                ...
                <div class="flex items-center space-x-2">
                    ...
                    <template v-for="(page, index) in paginationRange" :key="index">
                        <div v-if="page === '...'" class="flex h-8 w-8 items-center justify-center text-sm text-muted-foreground">...</div>
                        <Button
                            v-else
