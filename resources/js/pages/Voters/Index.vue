  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex flex-col gap-4 p-4">
      <div class="flex flex-col gap-4">
        <div class="flex justify-between items-center gap-2">
          ...
          <div class="space-x-2 flex items-center">
            <div class="relative w-full max-w-xs">
              ...
              <Input v-model="searchQuery" @input="handleSearch" type="text" placeholder="Search voters..."
                class="w-full pl-9 h-9" />
            </div>

            <input type="file" ref="fileInput" accept=".xlsx, .xls, .csv" class="hidden" @change="handleFileUpload" />

            <Button size="sm" @click="triggerFileInput" :disabled="loading" variant="outline"
        ...
        <div v-if="uploadStatus" :class="['p-3 rounded-lg border flex items-start gap-3 animate-fade-in',
          ...
          <div class="flex-1">
            <h4 class="font-medium mb-1" :class="uploadStatus.type === 'success' ? 'text-green-800' :
              uploadStatus.type === 'error' ? 'text-red-800' : 'text-blue-800'">
              {{ uploadStatus.title }}
            </h4>
            <p class="text-sm" :class="uploadStatus.type === 'success' ? 'text-green-700' :
        ...
        <div v-if="processingLargeFile" class="p-4 bg-blue-50 border border-blue-200 rounded-lg animate-pulse">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-sm text-blue-700">Please wait while we process {{ estimatedRowCount.toLocaleString() }}
                rows. The page will refresh automatically.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="voters.data.length > 0" class="flex flex-col sm:flex-row gap-4 justify-between items-center pt-2">
        <div class="text-sm text-muted-foreground">
          Showing <span class="font-medium text-foreground">{{ voters.from }}</span> to <span
            class="font-medium text-foreground">{{ voters.to }}</span> of <span class="font-medium text-foreground">{{
              voters.total }}</span> results
        </div>

        <div class="flex items-center space-x-2">
          <Button variant="outline" class="hidden h-8 w-8 p-0 lg:flex" :disabled="voters.current_page === 1"
            @click="handlePageChange(1)">
            <span class="sr-only">Go to first page</span>
            <ChevronsLeft class="h-4 w-4" />
          </Button>

          <Button variant="outline" class="h-8 w-8 p-0" :disabled="voters.current_page === 1"
          ...
          <template v-for="(page, index) in paginationRange" :key="index">
            ...
            <Button v-else :variant="voters.current_page === page ? 'default' : 'outline'" class="h-8 w-8 p-0"
              @click="handlePageChange(page as number)">
              {{ page }}
            </Button>
          </template>

          <Button variant="outline" class="h-8 w-8 p-0" :disabled="voters.current_page === voters.last_page"
            @click="handlePageChange(voters.current_page + 1)">
          ...
          <Button variant="outline" class="hidden h-8 w-8 p-0 lg:flex"
            :disabled="voters.current_page === voters.last_page" @click="handlePageChange(voters.last_page)">
