...
const candidatePositions = computed(() => {
    ...
    const grouped = election.candidates.reduce((acc: any, candidate) => {
        ...
        acc[posName].candidates.push({
            ...
            profile_pic: getImagePath(candidate.candidate_picture) || '/images/bg-only.jpg',
            platform_img: getImagePath(candidate.platform_image), 
        });
        return acc;
    }, {});
...
const handleImageError = (e: Event) => {
    ...
    target.src = '/images/bg-only.jpg';
};
</script>

<template>
    <Head title="Candidates' Platforms" />
    <div class="h-screen bg-cover bg-center bg-no-repeat relative flex flex-col overflow-hidden" style="background-image: url('/images/AU BG.jpg');">
        ...
        <main class="flex-grow overflow-y-auto custom-scrollbar">
            <div class="flex flex-col items-center w-full max-w-7xl gap-4 mx-auto py-6 px-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 pb-32 w-full justify-items-center">
                    <Card v-for="posData in candidatePositions" :key="posData.position"
                        ...
                        <div class="relative z-10 p-5 flex flex-col h-full justify-between">
                            <div>
                                ...
                                <CardContent class="p-0">
                                    <div class="space-y-3">
                                        ...
                                        <div v-for="candidate in posData.candidates" :key="candidate.name" class="flex items-center gap-2">
                            ...
                            <div class="mt-6">
                                <Button @click="openPopup(posData)" variant="outline"
        ...
        <div v-if="selectedPosition" @click.self="closePopup" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 p-4 overflow-y-auto">
            <div class="relative w-full max-w-6xl my-auto rounded-3xl overflow-hidden bg-cover bg-center border border-white/20 p-6 md:p-10 shadow-2xl"
                :style="{ backgroundImage: `url('/images/bg-only.jpg')` }">
                ...
                <div class="relative z-10 w-full flex flex-col items-center">
                    ...
                    <div class="grid w-full gap-8 h-full overflow-y-auto max-h-[65vh] custom-scrollbar px-2"
                        ...
                        <div v-for="candidate in selectedPosition.candidates" :key="candidate.name" 
                            ...
                            <div class="flex flex-col items-center gap-3">
                                <div class="w-24 h-24 md:w-28 md:h-28 border-2 border-purple-500/50 rounded-full overflow-hidden shadow-xl">
                                    <img :src="candidate.profile_pic" @error="handleImageError" class="w-full h-full object-cover" />
                                </div>
                            ...
                            <div class="w-full flex flex-col gap-2">
                                ...
                                <div v-if="candidate.platform_img" @click="openZoom(candidate.platform_img)"
        ...
        <Button @click="goToVoting" class="fixed bottom-24 right-6 bg-purple-600 hover:bg-purple-700 text-white font-bold px-6 py-4 rounded-full shadow-2xl transition-all hover:scale-105 z-50 uppercase tracking-widest text-xs">
            Go to Voting Page
        </Button>
